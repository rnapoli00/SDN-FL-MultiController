...
# --- LOGICA DI UNIONE E SHUFFLE DATASET ---

controllerid = int(sys.argv[1])
base_path = os.path.dirname(os.path.abspath(__file__))

# Lista dei file attesi dai 3 controller
csv_files = [
    os.path.join(base_path, "networkdatasetcontroller1.csv"),
    os.path.join(base_path, "networkdatasetcontroller2.csv"),
    os.path.join(base_path, "networkdatasetcontroller3.csv")
]
...
print(f"\n[Client {controllerid}] Tutti i dataset sono pronti. Inizio unione...")

# Leggiamo e uniamo i file (ora siamo sicuri che esistano e siano grandi abbastanza)
list_of_dfs = [pd.read_csv(f) for f in csv_files]
df_total = pd.concat(list_of_dfs, ignore_index=True)

# Mischiamo il dataset
df_total = df_total.sample(frac=1, random_state=42).reset_index(drop=True)

if controllerid == 1:
    combined_csv_path = os.path.join(base_path, "networkdataset_combined.csv")
    df_total.to_csv(combined_csv_path, index=False)
    print(f"[Client {controllerid}] Dataset unificato salvato con successo in: {combined_csv_path}")

# Dividiamo il dataset unito in 3 parti
total_rows = len(df_total)
chunk_size = total_rows // 3

start_idx = (controllerid - 1) * chunk_size
if controllerid == 3:
    df_processed = df_total.iloc[start_idx:]
else:
    df_processed = df_total.iloc[start_idx : start_idx + chunk_size]

print(f"[Client {controllerid}] Addestramento su {len(df_processed)} campioni estratti dal dataset globale di {len(df_total)} righe.")

# --- FINE LOGICA DI UNIONE ---
