...
class Controller2(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.X_local = []
        self.y_local = []
        self.model = SGDClassifier(max_iter=1, tol=None)  # modello leggero


        #threading.Thread(target=self.start_fl_client, daemon=True).start()
        p = multiprocessing.Process(target=self.start_fl_client)
        p.start()
...
print(os.getcwd())
df_processed = read_csv_files("../../Desktop/project/dataset/new_client2.csv")
df_processed
...
# (Traning does not know the target but testing will test it)
...
class NeuralNetwork(nn.Module):
    def __init__(self, input_size=115, hidden_size=50, num_layers=2, bidirectional=False):
        super().__init__()
    ...
    def forward(self, x):
        x = x.unsqueeze(1)  # Adding sequence dimension if necessary
        lstm_out, _ = self.lstm(x)
        x = lstm_out[:, -1, :]  # Take last LSTM output
        x = self.fc(x)
...
client2 = FlowerClient(model_dnn, trainloader, valloader, loss_fun, optimizer, epoch=5)


fl.client.start_client(
    server_address = "192.168.56.105:3333",
    client=client2,
)
...
def start_fl_client(self):
    ...
    class FLClient(fl.client.NumPyClient):
        def get_parameters(self, config):
                return [val for val in self.model.coef_.tolist()] + [val for val in self.model.intercept_.tolist()]

        def fit(self, parameters, config):
                if len(self.X_local) < 5:
                    return self.get_parameters(config), 0, {}
                X = np.array(self.X_local)
                y = np.array(self.y_local)
        ...
        def evaluate(self, parameters, config):
                ...
                return float(1.0 - acc), len(X), {}

    print("Starting Flower client...")
    fl.client.start_client(server_address="localhost:8080", client=FLClient())
'''
