...
def evaluation(confmat_glb):
    ...
    return [[recall_benign, precision_benign, f1_score_benign],
            [recall_ack,    precision_ack,    f1_score_ack],
            [recall_syn,    precision_syn,    f1_score_syn],
            [recall_fin,    precision_fin,    f1_score_fin],
            [recall_udp,    precision_udp,    f1_score_udp]]
...
def display_evaluation(eval_list):
    ...
    labels = ['benign', 'ack', 'syn', 'fin', 'udp']
    for label, values in zip(labels, eval_list):
...
def test(dataloader, model, loss_fn):
    ...
    with torch.no_grad():
        for tup in dataloader:
            ...
            confmat_glb += confmat(pred_int, y)
